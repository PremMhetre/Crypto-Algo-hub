...
export function startBitcoinStream() {
  ...
  ws.on("open", () => {
    console.log("🟠 Bitcoin stream connected");
  });
  ...
  ws.on("message", async (data) => {
    ...
    if (currentSecond !== null && sec !== currentSecond) {
      await saveBTC(bucket, currentSecond);
      bucket = [];
    }
...
async function saveBTC(trades, second) {
